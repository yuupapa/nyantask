...
**各タスク実装完了後に必ずCodex CLIでバグチェックを実施すること。**

### Codexチェックの手順

1. 実装ファイルを特定する
2. Codex に以下の観点でレビューを依頼する：
   - 型エラー・null安全性の問題
   - Server Action / Server Component の誤用
   - RLS・認証漏れ
   - エラーハンドリングの欠落
   - 本番環境で落ちうる箇所
...
| 観点 | チェック内容 |
|------|------------|
| TypeScript | 型エラー、any の濫用、null/undefined 未処理 |
| 認証 | requireAuth / requireAdmin の呼び忘れ |
| RLS | 新テーブルにRLSが設定されているか |
| エラー処理 | throw / return の使い分け、ユーザーへの適切なフィードバック |
| Server/Client | "use server" / "use client" の正しい使い分け |
| 本番リスク | 環境変数依存、外部API呼び出し失敗時の挙動 |
